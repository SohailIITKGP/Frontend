...
const url = `http://127.0.0.1:8000/api/jsondata/?end_year=${filter.end_year}&topic=${filter.topic}&sector=${filter.sector}&region=${filter.region}&pestle=${filter.pestle}&source=${filter.source}&country=${filter.country}&quantity=${filter.quantity}&pagination=${filter.pagination}&sortBy=${filter.sortBy}`;
    ...
    <>
      ...
      <button className="btn btn-primary" onClick={handleApplyFilters}>
        ...
        <table className="table table-bordered table-hover">
          <thead>
            <tr>
            <th key={1} onClick={() => handleSortBy('end_year')}>End year</th>
              <th key={2} onClick={() => handleSortBy('topic')}>Topics</th>
              <th key={3} onClick={() => handleSortBy('sector')}>Sector</th>
              <th key={4} onClick={()  => handleSortBy('region')}>Region</th>
              <th key={5} onClick={() => handleSortBy('pestle')}>Pest</th>
              <th key={6} onClick={() => handleSortBy('source')}>Source</th>
              <th key={7} onClick={() => handleSortBy('country')}>Country</th>
            </tr>
          </thead>
